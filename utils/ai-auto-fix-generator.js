...
async function gatherCodeContext(finding, workspaceDir) {
  ...
  try {
    ...
    console.log(
      `Warning: Could not gather full context for ${finding.file}: ${error.message}`,
    );
  }
...
async function detectProjectPatterns(workspaceDir) {
  ...
  try {
    ...
    const camelCaseFiles = sampleFiles.filter((f) =>
      /[a-z][A-Z]/.test(f),
    ).length;
...
function generateContextualSuggestions(finding, context) {
  ...
  if (finding.cwe === "CWE-89" || finding.title.toLowerCase().includes("sql")) {
    if (
      projectPatterns.framework === "express" &&
      projectPatterns.dependencies?.includes("prisma")
    ) {
      suggestions.push("Use Prisma client for type-safe database queries");
    } else if (projectPatterns.dependencies?.includes("sequelize")) {
      suggestions.push(
        "Use Sequelize parameterized queries with bound parameters",
      );
  ...
  if (finding.cwe === "CWE-79" || finding.title.toLowerCase().includes("xss")) {
    if (projectPatterns.framework === "react") {
      suggestions.push(
        "Remove dangerouslySetInnerHTML, use React text interpolation",
      );
    } else if (projectPatterns.framework === "vue") {
      suggestions.push(
        "Use v-text directive instead of v-html for user content",
      );
    } else {
...
function generateAgentPrompt(finding, context) {
  ...
  const codeSnippet = targetFile?.surroundingLines?.join("\n") || "";
  const lang = targetFile?.language || "typescript";
  const framework = projectPatterns?.framework;
  const deps = (projectPatterns?.dependencies || []).slice(0, 10).join(", ");

  const lines = [
    `I have a **${finding.severity}** severity finding in my codebase that needs a fix:`,
    ``,
  ...
  if (codeSnippet) {
    lines.push(``, `**Relevant code (around line ${finding.line || 1}):**`);
    lines.push(`\`\`\`${lang}`, codeSnippet, `\`\`\``);
  }

  if (finding.fix) {
    lines.push(``, `**Suggested approach:** ${finding.fix}`);
  }
  ...
  lines.push(
    ``,
    `Please provide the exact code fix for this issue. Show only the corrected code (or a unified diff). Explain briefly why the change prevents the problem.`,
  );

  return { copilotChatPrompt: lines.join("\n") };
}
...
async function generateIntelligentMockFix(finding, context) {
  ...
  if (
    ...
    finding.title.toLowerCase().includes("handling")
  ) {
...
function generateAnyTypeFix(finding, context) {
  ...
  if (
    surroundingCode.includes("req.body") ||
...
function generateSecretFix(finding, context) {
  return {
    analysis:
      "Hardcoded secrets pose security risks if source code is exposed.",
...
function generateErrorHandlingFix(finding, context) {
  return {
    analysis:
      "Inadequate error handling can lead to application crashes and poor user experience.",
    ...
    alternatives:
      "Consider using error boundary patterns or global error handlers.",
    confidence: "high",
    complexity: "moderate",
  };
}
 ...
 */
async function generateAIAutoFixes(
  resultsPath,
  workspaceDir,
  agentMode = true,
) {
  console.log(
    `🤖 Preparing findings for agent-driven auto-fixes: ${resultsPath}`,
  );
  ...
  for (const result of data.results) {
    for (const finding of result.findings) {
      ...
      // Also re-process agent-context findings that predate the copilotChatPrompt format
      const missingCopilotPrompt =
        finding.autofix?.type === "agent-context" &&
        !finding.autofix?.agentPrompt?.copilotChatPrompt;
      const needsEnhancement =
        missingCopilotPrompt ||
        !finding.autofix ||
      ...
      if (needsEnhancement && shouldGenerateAIFix(finding)) {
        ...
        try {
          ...
          if (agentData && agentData.requiresAgentAnalysis) {
            finding.autofix = {
              type: "agent-context",
              description: "Open in Copilot Chat for an AI-generated fix",
            ...
            enhancedCount++;
            console.log(
              `✨ Enhanced fix for: ${finding.title.substring(0, 60)}...`,
            );
          }
        } catch (error) {
          console.log(
            `⚠️  Failed to generate AI fix for ${finding.title}: ${error.message}`,
          );
        }
      }
    }
  ...
  console.log(
    `🎉 AI enhancement complete: ${enhancedCount}/${totalProcessed} findings enhanced`,
  );
...
function shouldGenerateAIFix(finding) {
  ...
  return (
    highValueCategories.includes(finding.category) ||
    highValueRules.some(
      (rule) =>
        finding.rule?.includes(rule) ||
        finding.title.toLowerCase().includes(rule),
    )
  );
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!resultsPath || !workspaceDir) {
    console.error(
      "Usage: node ai-auto-fix-generator.js <resultsPath> <workspaceDir> [openaiApiKey]",
    );
    console.error("");
    console.error("Examples:");
    console.error(
      "  node utils/ai-auto-fix-generator.js reports/owner_repo/audit-123/results.json workspace/owner_repo",
    );
    console.error(
      "  node utils/ai-auto-fix-generator.js reports/owner_repo/audit-123/results.json workspace/owner_repo sk-...",
    );
    process.exit(1);
  }
