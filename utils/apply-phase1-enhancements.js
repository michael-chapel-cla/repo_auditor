...
async function applyPhase1Enhancements(resultsPath, reportsDir, workspaceDir) {
  ...
  // Normalize 'repo' → 'repoFullName' if agent wrote the wrong key
  try {
    const raw = JSON.parse(await readFile(resultsPath, "utf8"));
    if (raw.repo && !raw.repoFullName) {
      raw.repoFullName = raw.repo;
      delete raw.repo;
      const { writeFile } = await import("fs/promises");
      await writeFile(resultsPath, JSON.stringify(raw, null, 2));
      console.log(
        `⚠️  Normalized field: "repo" → "repoFullName" (${raw.repoFullName})`,
      );
    }
  } catch (_) {
    /* non-fatal */
  ...
  try {
    const basicFixableCount = await generateAutoFixes(
      resultsPath,
      workspaceDir,
    );
  ...
  console.log(
    "🤖 1.2b - Preparing findings for agent-driven auto-fix suggestions...",
  );
  try {
    console.log("🧠 Using agent LLM context instead of external API calls");

    const aiEnhancedCount = await generateAIAutoFixes(
      resultsPath,
      workspaceDir,
    );
  ...
  if (workspaceDir) {
    ...
    try {
      const attributedCount = await applyContributorRiskAttribution(
        resultsPath,
        workspaceDir,
      );
      ...
      results.contributorRiskAttribution = {
        success: false,
        error: error.message,
      };
    }
    ...
    console.log(
      "👥 2.4 - Skipping contributor risk attribution (no workspace provided)",
    );
    results.contributorRiskAttribution = {
      success: false,
  ...
  if (successCount < 4) {
    // Allow AI auto-fix to fail gracefully
    console.log("⚠️  Some enhancements failed - check logs above");
    if (successCount < 3) {
      // Only exit if critical enhancements fail
      process.exit(1);
    }
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!resultsPath) {
    console.error(
      "Usage: node apply-phase1-enhancements.js <resultsPath> [reportsDir] [workspaceDir]",
    );
    console.error("");
    console.error("Examples:");
    console.error(
      "  node utils/apply-phase1-enhancements.js reports/owner_repo/audit-123/results.json",
    );
    console.error(
      "  node utils/apply-phase1-enhancements.js reports/owner_repo/audit-123/results.json reports/ workspace/owner_repo",
    );
    process.exit(1);
  }
